...
func (rq *ReportQuery) QueryPost() *PostQuery {
...
func (rq *ReportQuery) QueryReportedBy() *UserQuery {
...
func (rq *ReportQuery) Clone() *ReportQuery {
	...
	return &ReportQuery{
		...
		withReportedBy: rq.withReportedBy.Clone(),
...
func (rq *ReportQuery) WithPost(opts ...func(*PostQuery)) *ReportQuery {
	query := (&PostClient{config: rq.config}).Query()
...
func (rq *ReportQuery) WithReportedBy(opts ...func(*UserQuery)) *ReportQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withReportedBy = query
	return rq
}
...
func (rq *ReportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Report, error) {
	...
	if query := rq.withPost; query != nil {
		if err := rq.loadPost(ctx, query, nodes,
			func(n *Report) { n.Edges.Post = []*Post{} },
			func(n *Report, e *Post) { n.Edges.Post = append(n.Edges.Post, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withReportedBy; query != nil {
		if err := rq.loadReportedBy(ctx, query, nodes,
			func(n *Report) { n.Edges.ReportedBy = []*User{} },
			func(n *Report, e *User) { n.Edges.ReportedBy = append(n.Edges.ReportedBy, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReportQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Report, init func(*Report), assign func(*Report, *Post)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Report)
	nids := make(map[int]map[*Report]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(report.PostTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(report.PostPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(report.PostPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(report.PostPrimaryKey[1]))
...
func (rq *ReportQuery) loadReportedBy(ctx context.Context, query *UserQuery, nodes []*Report, init func(*Report), assign func(*Report, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Report)
	nids := make(map[int]map[*Report]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(report.ReportedByTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(report.ReportedByPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(report.ReportedByPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(report.ReportedByPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "reported_by" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
