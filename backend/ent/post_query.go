...
func (pq *PostQuery) QueryImages() *PostImageQuery {
	query := (&PostImageClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) loadImages(ctx context.Context, query *PostImageQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostImage)) error {
