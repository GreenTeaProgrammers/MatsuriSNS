...
const MapPage: React.FC = () => {
  ...
  const pins = data
    .filter(item => {
      ...
      return itemTime <= sliderValue; // スライダーの時刻以下のピンを表示
    })
    .map((item, index) => ({
      id: `pin${index + 1}`,
      x: item.location_x,
  ...
  return (
    <div>
      ...
      <Map
        ...
        pins={pins.map(pin => ({
          ...pin,
          onClick: () => handlePinClick(pin.comment, pin.createdAt), // クリック時にコメントと時刻を渡す
        }))}
      />
      {selectedPost && (
        <div style={{ marginTop: '20px', padding: '10px', border: '1px solid #ccc' }}>
          ...
          <p><strong>投稿時刻:</strong> {new Date(selectedPost.createdAt).toLocaleString()}</p> {/* 投稿時刻の表示 */}
        </div>
      )}
      <TimeSlider
        minTime={minTime}
